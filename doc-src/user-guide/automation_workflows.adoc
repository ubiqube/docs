...
.Summary
This guide explains of to list the workflow, select them and run them.

A Workflows is composed of:

- A collection of processes, each process is composed of a set of executable tasks.
...
To run a Worflow you must first select a Customer from the Customer selection list at the top of the screen.

This will display the customer management dashboard for the Workflows.
This dashboard show the overall status of process execution
...
Before you can start using a Workflow you need to create a new instance of the Workflow (you can think of that as using a class to create a new object instance in object oriented programming - OOP).

The action on the top right will create a new instance and open a user form where you will be able to provide some parameters related to the creation of the instance (you can thnk of this as passing parameter to the contructor in OOP). 
The form may not always require parameter (this would be the case of the default constructor in OOP)
