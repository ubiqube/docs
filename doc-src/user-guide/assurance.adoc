...
[.center]
image:msa_arch_assurance.png[width=400px]
...
To update the configuration of your setup, use the `POST /system-admin/v1/msa_vars` REST API to update the configuration variables `UBI_SMTP_IPADDR` and `UBI_MAIL_SMTP` with the IP address of your SMTP server.
