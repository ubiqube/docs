...
The mini lab requires a valid product license in order to function properly. 

Whether you are running a free trial, an advanced PoC or a production {product_name} you need to get a license from UBiqube and link:../admin-guide/license_activation{outfilesuffix}[install it].
...
=== Step 3: install the libraries (Microservices, Workflows and Adapters from github.com/openmsa
...
=== Step 4: provision the mini lab with tenants, managed entities,...
...
=== Step 5 (optional): Setting up log aggregation for linux_me
