...
- minimum RAM memory to allocate to Docker: 6GB (8GB to be comfortable), 2 CPU, 50 GB of disk space.
...
4. `./script/install.sh` or `./script/install_win.sh` for Windows users
...
[source, shell]
----
[root@msa2 quickstart]# ./scripts/install.sh --help
usage: install.sh [--mini-lab|-m] [--force|-f] [--cleanup|-c] [--remove-orphans|-ro]
this script installs and upgrades a MSA
...
If you want to update your local quickstart git repository, because you have already checked out the release tag {revnumber} on a local git branch {revnumber}, you need first to delete this local repository before you can update the quickstart and install the hot fixes.
...
2. `cd quickstart`
3. `git fetch --depth 1 origin refs/tags/{revnumber}:refs/tags/{revnumber}`
4. `./script/install_win.sh`

NOTE : you may face an issue at the end of the script with a message "Wait Kibana to be ready". The time to fix this known issue, you can execute in another terminal docker-compose up -d to fix this problem.
