...
The easiest way to do that is to edit the file docker-compose.yml and add the following to the services `msa_dev`, `msa_api` and `msa_sms`:

----
environment:
    http_proxy: "<PROXY URL>"
    https_proxy: "<PROXY URL>"
    no_proxy: "localhost,127.0.0.1,linux_me_2,linux_me,msa_cerebro,camunda,msa_alarm,msa_db,msa_sms,quickstart_msa_ai_ml_1,msa_ui,msa_dev,msa_bud,msa_kibana,msa_es,msa_front,msa_api"
----
...
On link:https://ubiqube.com/free-trial/[ubiqube.com,window=_blank], fill the form to receive a trial license by email.
