...
```
$ sudo docker-compose exec msa_dev /usr/bin/install_libraries.sh all
```
...
This script is designed to polulate the libraries for a fresh install or to update your libraries with the latest version from Github.
...
In case of calling this script on an existing setup, it will take care of merging the code from the Github master branch into your local development branch. With the option -y, merge will be attempted automatically, without it, the script will ask for user input.
