...
==== Steps 1: upgrade the {product_name}
...
==== Steps 2: update the community librairies

Install the libraries (Microservices, Workflows and Adapters) from github.com/openmsa

```
$ sudo docker-compose exec msa_dev /usr/bin/install_libraries.sh all --no-lic
```

NOTE: if you don't use the option --no-lic, a new trial license will be installed. This will overwrite any existing license you may have installed separately.
