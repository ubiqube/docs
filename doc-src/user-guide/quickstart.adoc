...
- minimum memory: 6GB (8GB to be comfortable), 2CPU, 100GB

NOTE: with less that 6GB of memory, the {product_name} may not function properly: some container such as the API container may not be able to start causing issues when connecting to the UI.
...
1. `git clone -b {revnumber} --depth 1 https://github.com/ubiqube/quickstart.git`
2. `cd quickstart`
...
[#step{counter:step}]
=== Step {step}: the libraries (Microservices, Workflows and Adapters) from github.com/openmsa
...
NOTE: more explanations on the script `install_libraries.sh` can be found link:#install_libraries[here].

[#step{counter:step}]
=== Step {step}: configure Elasticsearch and Kibana

Configure Elasticsearch settings and index mappings

`docker exec -u root -w /home/install/scripts/ -it  msa_es bash -c './install.sh'`

Install Kibana visualizations and dashboards
...
1. `cd ..`
2. `git clone -b {revnumber} --depth 1 https://github.com/ubiqube/quickstart.git`
3. `cd quickstart`
...
==== Hot fixes: upgrade a version with an updated tag

Sometime, not very often, but it can happen in the few days following a release, we release some hot fixes on the same version.

When this happens, we update the docker images in the quickstart and simply move the release tags to the new git commit.

If you want to update your quickstart, because you have already checked out the release tag {revnumber} on a local git branch {revnumber}, you need first to delete this local branch and delete the local tag before you can update the quickstart and install the hot fixes.
