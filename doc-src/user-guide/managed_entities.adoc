...
A graph, based on ICMP requests from the CoreEngine container `msa_sms` to the management interface of the managed entity, shows the latency and TTL information.

When the connectivity fails, a event `VNOC-1-IPDOWN` is generated by the CoreEngine and indexed in Elasticsearch log index. An link:assurance{outfilesuffix}#alarm[alarm] can be configured based on this event

[#microservice]
== Configuration with microservices

Managed entities can be configured with link:./configuration_microservices{outfilesuffix}[microservices]. To access the microservice console, click on the tab "Configure" on the managed entity screen.
