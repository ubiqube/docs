...
*Managed Entities* are the logical representation of any element of an infrastruucture such as network devices, security devices, VNF, VIM,...
...
You can optionaly set the management interface name here. When set, the CoreEngine will attempt to use to poll the management interface traffic with SNMP.
...
.Log analytics
Check to collect syslogs and optionaly analyse the syslogs. The syslogs will be parsed and stored in the Elasctisearch cluster.
...
The link:https://github.com/openmsa/Adapters/tree/master/adapters/rest_generic[REST generic adapter] uses configuration variables to customise it's behavior and adapt itself to the various type of REST API (BASIC auth vs. Token auth, HTTP vs. HTTPS,...)
