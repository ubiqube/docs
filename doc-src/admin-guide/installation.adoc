= Installing {product_name} as a docker application
:imagesdir: ./resources/
...
[NOTE]
====
There might be a pop-up warning of a self-signed cert that need to be accepted to proceed.

Users might need to wait a few minutes after they access the localhost as it takes a few minutes for the {product_name} to start in the background before they logon
====