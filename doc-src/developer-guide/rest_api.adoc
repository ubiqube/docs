...
The MSA(TM) provides a support for REST API. These API can be used by third-party service, application or script to manage your MSA(TM) instance.

.Enabling REST API Support
The API is enabled by default. No additional configuration is required.

.Authentication
When making requests to MSA(TM) using the REST API, you will need:
...
.Format
MSA(TM) API uses the JSON format.


== Example API commands
For the full list of available commands, see the MSA(TM) API guide on your MSA(TM) instance: http://<MSA IP or FQDN>/swagger
