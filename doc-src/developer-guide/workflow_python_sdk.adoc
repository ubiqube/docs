...
try:
  ...
  MSA_API.task_error(f'CREATE ERROR: {str(e)}', context)
----
...
MSA_API.task_success(f'IPTABLES RULE INITIALIZED', 	context)
----
...
try:
  ...
  MSA_API.task_error(f'IMPORT ERROR: {str(e)}', context)
----
