...
.A new process
image:workflow_editor_new_process.png[width=700px]
...
The log files are generated per workflow instance in the container `msa_api` under `/opt/jboss/wildfly/standalone/logs/`. The log files are formatted as `process-XX.log` where XX is the workflow instance ID.

.39 is the workflow instance ID
image:workflow_instance_id.png[width=700px]

You can monitor the logs of a process with the CLI command below
