...
It might take a bit of time for the execution to display the results, but a lot is happening in the backend, which we’ve made easy for you at the frontend, so don’t worry. It will end up being successful much quicker than if this were done manually. (Also, less prone to error.)
