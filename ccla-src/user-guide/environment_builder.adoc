...
Same as the blueprint editor, a user can modify a design before to deploy it. Two operations are possible, additionally to the save design :
...
The *Dashbord* display various informations about the entire environment. It can be personalized as explained in xref:navigation_dashboard.adoc[Navigation and Dashboard settings]
...
* *FinOps :* Displays environment cost by providers. _Refers to_ xref:cost_management.adoc[_Cost management page_]

* *Monitor :* Provide environment monitoring using Prometheus and Grafana (details below in Monitoring section).
