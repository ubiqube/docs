...
_Note : this screen can generate lots of queries if multiple environments are selected at the same time_

_Note : the tag section will be implemented in future version_

== Estimate in quick deployment ==

In quick deployment, before to deploy, the user can see what is the estimate cost of this entire deployment.The price is based on the environment used.
